...
- **음료 그래픽 에셋(18종)** — 생성형으로 소싱, `assets/drinks/{play,card,mini}/`에 배선 완료.
  다만 지금 파일은 시트에서 추출·업스케일한 임시본이라 재생 카드에서 뿌옇게 보인다 —
  네이티브 1536×1024로 재생성해서 파일만 교체하면 됨. 상세는 **9-1** 참고
...
- **용량**: 라이브러리 그리드는 18장이 한 번에 뜨므로 600×516 한 장당 **40KB 이하**.
  (현재 실측: play 합계 1,147KB·최대 85KB / card 614KB·최대 46KB / mini 135KB·최대 10KB)

#### 빌드 & 배선

```
python3 tools/build-drink-assets.py <마스터폴더>     # → assets/drinks/{play,card,mini}/*.webp
```

마스터에서 3종을 중앙 크롭·리샘플·WebP 변환한다. 가장자리에 밝고 무채색인 띠가 있으면
(시트에서 셀을 잘라 만든 마스터에서 생김) 자동으로 감지해 대칭으로 잘라낸다.

- 파일명은 `DRINKS`의 `slug` 필드와 1:1로 맞춘다. slug는 이름에서 유추하지 않고 직접 박아둔다 —
  유추하면 음료 이름을 바꿀 때 조용히 깨진다.
- 렌더는 `setDrinkImg()` 하나로 통일. 이미지는 이모지 위에 덮이고, **로드에 성공해야만** `.on`이
  붙어 보인다. 에셋이 없거나 실패하면 뒤에 깔린 기존 이모지가 그대로 노출된다(폴백).
- 이 변경 전에 저장된 라이브러리 항목에는 `slug`가 없다 → `itemSlug()`가 이름으로 역조회해서 메운다.
- 라이브러리 카드 이미지는 `loading="lazy"` — 18장이 한 번에 뜨므로.
- 서비스워커는 동일 출처 GET을 런타임에 캐시하므로 `sw.js`의 `SHELL`에 넣지 않는다
  (설치 시점에 54개를 받게 되어 설치가 느려진다).

#### ⚠️ 현재 에셋은 임시본 — 재생성 필요

지금 들어간 18종은 생성기가 **6열 × 3행 시트**로 반환한 걸 셀 단위로 추출해 1536×1024로 업스케일한
것이다. 파일 규격은 정확하지만 **유효 해상도가 가로 ~256px**(명목 1536px, 6배 업스케일)뿐이다.
1/2로 줄였다 되돌렸을 때 오차가 0.6인데, 네이티브 이미지는 같은 조건에서 10~14가 나온다.

| 용도 | 표시 | 실제 디테일 | 배율 | 판정 |
|---|---|---|---|---|
| mini | 52pt | ~171px | 3.3x | 충분 |
| card | 177pt | ~198px | 1.1x | 1x급, 레티나에서 물렁함 |
| play | 374pt | ~256px | **0.7x** | **1x도 안 됨 — 재생성 필요** |

재생성할 때는 **한 메시지에 한 장씩** 요청해야 한다. 여러 장을 한 번에 요구하면 컨택트 시트로
반환되고 위 문제가 그대로 재발한다. 프롬프트에 명시할 것:
`Generate exactly ONE image at 1536x1024 landscape. Do not create a grid, sheet, or collage.`
...
1. ~~**9-1 음료 18종**~~ — 배선 완료, **에셋 재생성만 남음**(위 경고 참고)
2. **9-2 아이콘 PNG** — 소스가 이미 있어서 반나절
3. **9-3 OG** — 1~2장
4. 9-4 → 9-5
