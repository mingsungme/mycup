...
`grind`(원두 갈기)와 `steam`(김)은 **Web Audio 합성**을 그대로 쓴다(파일 없음).
합성 얼음·물소리는 실물과 달라 녹음으로 교체했지만, 그라인더와 스팀은 노이즈
기반이라 합성이 충분히 그럴듯하다. 파일이 늘지 않는 이점도 있다.

| 소리 | 방식 |
|---|---|
| ice · pour · soda | 녹음 (이 폴더) |
| grind · steam | Web Audio 합성 (`js/app.js`) |
