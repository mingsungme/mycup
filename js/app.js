...
function buildProfile(s) {
  ...
  return { name: drink.name, emoji: drink.emoji, color: drink.color, cupColor,
           code, sub: drink.sub, sliders: { ...s }, hot,
...
function demoQueue(profile) {
  return (DEMO_SETS[profile.vibe] || DEMO_SETS['chill lofi beats']).map((id) => ({
    videoId: id,
...
function renderReceipt() {
  ...
  $('receipt-music-rows').innerHTML = [
    ...
    ['SOURCE', state.demo ? 'Demo Curation' : 'YouTube Match'],
...
function init() {
  ...
  $('btn-key-save').addEventListener('click', () => {
    const v = $('input-yt-key').value.trim();
    if (v) { localStorage.setItem(LS_KEY, v); toast('API 키가 저장됐어요 (localStorage)'); }
    else toast('키를 입력해 주세요');
    $('modal-settings').classList.add('hidden');
  });
  $('btn-key-clear').addEventListener('click', () => {
    localStorage.removeItem(LS_KEY);
    $('input-yt-key').value = '';
    toast('저장된 키를 삭제했어요 — 데모 모드로 동작합니다');
  });
